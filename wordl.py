...
jalap""".splitlines()
def loop():
    words = [line.strip().lower() for line in wordz if len(line.strip())==5]
    print()
    all_guesses = input().lower()
    ...
    confirmed_letters = set(j for _,j in greens+yellows)
    used_letters = set(all_guesses)
    gray_letters = used_letters - confirmed_letters
    possible=[]
    for word in words:
        if any(word[i]!=j for i,j in greens):continue
        if any(word[i]==j or j not in word for i,j in yellows):continue
        if any(j in gray_letters for j in word):continue
        possible.append(word)
    print(f"Possible words ({len(possible)}):")
    for w in possible:print(w)
    loop()
loop()